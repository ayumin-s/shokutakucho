...
function renderTable(){
  ...
  html += `<div class="bar"><input id="tq" placeholder="料理・お店・タグで探す" value="${esc(S.q)}"></div>`;

  if(!rows.length){
    html += `<div class="empty">${S.meals.length ? '見つかりませんでした' : 'まだ記録がありません。<br>右下の＋から始めましょう。'}</div>`;
  }else{
    let cur = '';
    rows.forEach(m => {
      const mk = monthKey(m.date);
      if(mk !== cur){ cur = mk; html += `<div class="month">${mk}</div>`; }
      const shop = shopOf(m);
      html += `<article class="meal" data-meal="${m.id}">
        ...
        <div class="foot">
          ...
          ${m.rating ? `<span style="margin-left:auto;color:var(--rose);letter-spacing:.1em;">${'★'.repeat(m.rating)}</span>` : ''}
        </div>
      </article>`;
    });
  }
  ...
  v.innerHTML = html;
  bindOwnerSwitch(v);
  const inp = v.querySelector('#tq');
  if(inp) inp.oninput = e => { S.q = e.target.value; renderTable(); inp.focus(); };
...
function renderShops(){
  ...
  if(!rows.length){
    ...
    rows.forEach(s => {
      ...
      html += `<article class="shop" data-open="${s.id}">
        ...
        <div class="acts">
          ${s.url ? `<a href="${esc(s.url)}" target="_blank" rel="noopener" data-stop>MAP</a>` : ''}
  ...
  const sq = v.querySelector('#sq');
  if(sq) sq.oninput = e => { S.q = e.target.value; renderShops(); v.querySelector('#sq').focus(); };
...
function openShop(id){
  ...
  if(s.photo) html += `<img src="${s.photo}" style="width:100%;height:170px;object-fit:cover;display:block;margin-bottom:16px;">`;
  ...
  html += `<div class="shop"><div class="acts" style="padding-bottom:4px;">
    ${s.url ? `<a href="${esc(s.url)}" target="_blank" rel="noopener">MAP</a>` : ''}
  ...
  if(visits.length){
    ...
    visits.forEach(m => {
      html += `<div class="row" data-m="${m.id}">
        ${m.photo ? `<img src="${m.photo}" style="width:46px;height:46px;object-fit:cover;flex:0 0 auto;">` : ''}
...
function renderMealForm(){
  ...
  renderPhotoBox($('fPhoto'), f.photo, p => { collectMeal(); S.form.photo = p; renderMealForm(); });
...
function collectMeal(){
  const g = id => $(id)?.value ?? '';
  S.form = { ...S.form, title:g('fTitle'), date:g('fDate'), memo:g('fMemo'), url:g('fUrl'), tags:g('fTags'),
...
async function saveMeal(){
  ...
  const row = { user_id:S.me.id, kind:f.kind, title:f.title.trim(), date:f.date || today(),
    shop_id:f.shop_id || null, memo:f.memo.trim(), url:f.url.trim(), tags:f.tags,
    rating:f.rating||0, photo:f.photo };
  if(f.id) row.id = f.id;
  $('fSave').disabled = true;
...
function openShopForm(s, backToMeal=false){
  S.form = { name:'', scenes:[], area:'', memo:'', url:'', tags:'', priority:'someday',
...
function renderShopForm(){
  ...
  v.innerHTML = `
    ...
    <div class="field"><label>地図・サイトのURL</label>
  ...
  renderPhotoBox($('sPhoto'), f.photo, p => { collectShop(); S.form.photo = p; renderShopForm(); });
...
async function saveShop(){
  ...
  const row = { user_id:S.me.id, name:f.name.trim(), scenes:f.scenes, area:f.area.trim(),
    memo:f.memo.trim(), url:f.url.trim(), tags:f.tags, priority:f.priority, state:f.state||'wish', photo:f.photo };
  if(f.id) row.id = f.id;
  $('sSave').disabled = true;
...
// ---- 写真・星の共通部品 --------------------------------------
function renderPhotoBox(host, photo, cb){
  if(photo){
    host.className = 'photowrap';
    host.innerHTML = `<img src="${photo}" alt=""><button>✕</button>`;
    host.querySelector('button').onclick = () => cb(null);
  }else{
    host.className = '';
    host.innerHTML = `<button class="photobox">＋ 写真を追加</button>`;
    host.querySelector('button').onclick = () => pickPhoto(cb);
  }
}
